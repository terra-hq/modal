class c {
   ...
   */
  constructor(t = {}) {
    if (!(t.selector instanceof Element)) {
      console.warn("[Modal] selector must be a DOM Element. Modal not initialized."), this._disabled = !0;
      return;
    }
    const n = {
      onShow: (e) => {
        t.debug && console.info(`${e.id || "[no-id]"} is shown`);
      },
      onClose: (e) => {
        t.debug && console.info(`${e.id || "[no-id]"} is hidden`);
      },
      beforeOpen: () => {
      },
      beforeClose: () => {
    ...
    this.settings = { ...n, ...t }, this._safe = (e, ...s) => {
      ...
      if (console.error(`[Modal] ${s}`, i), typeof ((o = this.settings) == null ? void 0 : o.onError) == "function")
        try {
          this.settings.onError(new Error(s), { method: e, ...i });
        } catch {
        }
    }, this.DOM = {
  ...
  logDebug(t) {
    this.settings.debug && console.debug(`[Modal Debug]: ${t}`);
  }
  init() {
    if (!this.DOM.modal)
  ...
  addEventListeners(t) {
    ...
    }), t.querySelectorAll(`[${this.settings.closeTrigger}]`).forEach((s) => {
      const i = (o) => {
        var l;
    ...
    const e = t.querySelector("[data-modal-overlay], .c--modal-a__overlay");
    if (e) {
      const s = (i) => {
        if (i.target === e) {
          this.logDebug(`Overlay clicked for modal ID: ${n}`);
          const o = { type: "overlay", element: e, action: "close" };
          this.close(t, o);
        }
      };
  ...
  _ensureEscAttached() {
    this._attachEsc || (this._attachEsc = (t) => {
      t.key === "Escape" && this.isOpen(this.DOM.modal) && this.close(this.DOM.modal, { type: "keyboard", key: "Escape", action: "close" });
  ...
  open(t, n = null) {
    ...
    const e = t instanceof Element ? t : (i = this.DOM) == null ? void 0 : i.modal;
    ...
    if (this._busy || this.isOpen(e)) return;
    this._busy = !0;
    const s = n || { type: "programmatic", source: "manual", method: "open()" };
    this._safe(this.settings.beforeOpen, e, s), this.logDebug(`Opening modal ID: ${e.id}`), e.classList.add(this.settings.openClass), e.setAttribute("aria-hidden", "false"), this.settings.disableScroll && (document.body.style.overflow = "hidden"), this._ensureEscAttached(), this._safe(this.settings.onShow, e, s), this._busy = !1;
  }
  close(t, n = null) {
    var i;
    if (this._disabled) {
    ...
    this._safe(this.settings.beforeClose, e, s), this.logDebug(`Closing modal ID: ${e.id}`), e.classList.remove(this.settings.openClass), e.setAttribute("aria-hidden", "true"), this.settings.disableScroll && (document.body.style.overflow = ""), this._removeEsc(), this._safe(this.settings.onClose, e, s), this._busy = !1;
  }
  isOpen(t) {
    return t.classList.contains(this.settings.openClass);
  }
  toggle() {
    var t;
    if (!((t = this.DOM) != null && t.modal)) {
  ...
  destroy() {
    this._destroyed || (this.logDebug("Destroying all event listeners."), this.eventListeners.forEach(({ element: t, type: n, listener: e }) => {
